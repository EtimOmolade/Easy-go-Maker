      ...
      <DropdownMenuContent align="end" className="w-80 p-0 bg-background">
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex gap-2">
            ...
            {unreadCount > 0 && <Button variant="ghost" size="sm" onClick={handleMarkAllAsRead} className="h-8 text-xs mx-0 px-0">
                <CheckCheck className="h-3 w-3 mr-1" />Mark all read
              </Button>}
          </div>
        </div>
        <ScrollArea className="h-[400px]">
          {notifications.length === 0 ? <div className="flex flex-col items-center justify-center p-8 text-center">
              <Bell className="h-12 w-12 text-muted-foreground/50 mb-3" />
              <p className="text-sm text-muted-foreground">No notifications yet</p>
            </div> : <div className="divide-y">
              {notifications.map(notification => <button key={notification.id} onClick={() => handleNotificationClick(notification)} className={`w-full text-left p-4 hover:bg-accent transition-colors relative group ${!notification.is_read ? 'bg-primary/5' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => handleDeleteNotification(notification.id, e)}>
                            <X className="h-3 w-3" />
                          </Button>
                        </div>
                      </div>
