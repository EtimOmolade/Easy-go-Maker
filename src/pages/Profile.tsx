...
const Profile = () => {
  ...
  useEffect(() => {
    if (profile) {
      setTutorialEnabled(!profile.tutorial_completed);
    } else {
  ...
  const fetchProfile = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("profiles").select("name, email, streak_count, reminders_enabled, two_factor_enabled, voice_preference, onboarding_completed, tutorial_completed").eq("id", user.id).single();
      if (error) {
        ...
        <CardContent>
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <div className="flex items-center justify-between space-x-2">
              ...
              <Switch id="tutorial" checked={tutorialEnabled} onCheckedChange={async checked => {
                ...
                if (checked && user) {
                  try {
                    ...
                    console.log('🚀 Profile: Initiating redirect to /dashboard');
                    setTimeout(() => {
                      console.log('🚀 Profile: Executing navigate("/dashboard")');
                      navigate("/dashboard");
                    }, 1500);
                  } catch (error) {
