-- Phase 1: Database Schema Migration for SpiritConnect Backend (Clean Version)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'testimonies_journal_entry_id_fkey'
  ) THEN
    ALTER TABLE public.testimonies 
    ADD CONSTRAINT testimonies_journal_entry_id_fkey 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'testimonies_status_check'
  ) THEN
    ALTER TABLE public.testimonies 
...
-- Add check constraint for prayer_library day_of_week
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'prayer_library_day_of_week_check'
  ) THEN
    ALTER TABLE public.prayer_library 
    ADD CONSTRAINT prayer_library_day_of_week_check 
...
-- Add unique constraint for daily_prayers
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'daily_prayers_user_guideline_day_unique'
  ) THEN
    ALTER TABLE public.daily_prayers 
    ADD CONSTRAINT daily_prayers_user_guideline_day_unique 
    UNIQUE(user_id, guideline_id, day_of_week);
  END IF;
END $$;

-- Add check constraint for daily_prayers day_of_week
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'daily_prayers_day_of_week_check'
  ) THEN
    ALTER TABLE public.daily_prayers 
    ADD CONSTRAINT daily_prayers_day_of_week_check 
    CHECK (day_of_week IN ('monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'));
  END IF;
END $$;
...
-- ============================================
-- 4. CREATE RLS POLICIES (Drop and recreate to avoid conflicts)
-- ============================================

-- Prayer Library Policies
DROP POLICY IF EXISTS "Admins can manage prayer library" ON public.prayer_library;
CREATE POLICY "Admins can manage prayer library"
ON public.prayer_library
FOR ALL
USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Everyone can view prayer library" ON public.prayer_library;
...
DROP POLICY IF EXISTS "Users can view own daily prayers" ON public.daily_prayers;
CREATE POLICY "Users can view own daily prayers"
ON public.daily_prayers
FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create own daily prayers" ON public.daily_prayers;
CREATE POLICY "Users can create own daily prayers"
ON public.daily_prayers
FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all daily prayers" ON public.daily_prayers;
